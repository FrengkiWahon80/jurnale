...
def init_db():
    ...
    c = conn.cursor()
    # Tabel Guru / User (Kelas dihapus, default sekolah SMPN Tujuh Maret Hadakewa)
    c.execute('''
...
def generate_summary(nama_siswa, logs):
    ...
    Jika tidak ada, akan otomatis menggunakan Template Narasi Cerdas tanpa API key.
    """
    ...
    if api_key:
        try:
            ...
            st.warning(f"Sistem AI mengalami kendala ({e}). Mengalihkan ke generator narasi internal...")

    # FALLBACK NARRATIVE (Jika Tidak Ada API Key)
    kebiasaan_list = [f"• Tanggal {l[0]}: {l[1]}" for l in logs if l[1]]
    dimensi_list = [f"• Tanggal {l[0]}: {l[2]}" for l in logs if l[2]]
    umum_list = [f"• Tanggal {l[0]}: {l[3]}" for l in logs if l[3]]

    narasi = f"**RANGKUMAN EVALUASI PERKEMBANGAN SISWA**\n\n"
    narasi += f"Yth. Bapak/Ibu Orang Tua/Wali dari **{nama_siswa}**,\n\n"
    narasi += f"Berikut adalah ringkasan perkembangan ananda selama periode pencatatan harian di sekolah:\n\n"
    
    narasi += "📌 **1. Perkembangan 7 Kebiasaan Anak Indonesia:**\n"
    if kebiasaan_list:
        narasi += "\n".join(kebiasaan_list) + "\n\n"
    else:
        narasi += "Ananda menunjukkan sikap umum yang cukup baik dalam pembiasaan harian.\n\n"
        
    narasi += "📌 **2. Perkembangan 8 Dimensi Lulusan:**\n"
    if dimensi_list:
        narasi += "\n".join(dimensi_list) + "\n\n"
    else:
        narasi += "Ananda terus berproses dalam menginternalisasi nilai-nilai karakter lulusan.\n\n"
    ...
    narasi += f"Secara umum, Ananda **{nama_siswa}** menunjukkan perkembangan karakter yang positif. Mohon dukungan Bapak/Ibu di rumah untuk terus memotivasi ananda dalam mempertahankan kebiasaan baik ini."
...
def generate_docx(nama_guru, sekolah, nama_siswa, nisn, summary_text, logs):
    ...
    doc.add_heading('1. Rangkuman Evaluasi Perkembangan (7 Kebiasaan & 8 Dimensi)', level=2)
...
# ==========================================
st.set_page_config(page_title="Sistem Laporan Siswa - SMPN 7 Maret Hadakewa", layout="wide")
...
if not st.session_state['logged_in']:
    ...
    if choice == "Login Guru":
        ...
        # Default Nama Sekolah diset otomatis ke SMPN Tujuh Maret Hadakewa
        nama_sekolah = st.text_input("Nama Sekolah", value="SMPN Tujuh Maret Hadakewa")
        ...
        if st.button("Daftar Akun"):
            if new_user and new_password and nama_guru:
                try:
                    ...
                    conn.commit()
                    conn.close()
                    st.success("Akun berhasil dibuat! Silakan pilih menu 'Login Guru' pada sidebar.")
...
# ==========================================
    ...
    tabs = st.tabs(["📋 Data Siswa", "✍️ Catatan Perkembangan Harian", "📄 Generate Laporan & Download Word"])
    ...
    with tabs[0]:
        ...
        st.info(f"Guru: **{guru_data['nama_guru']}** | Sekolah: **{guru_data['nama_sekolah']}**")
    ...
    with tabs[1]:
        ...
        if not students:
            st.warning("Silakan tambahkan siswa terlebih dahulu pada tab 'Data Siswa'.")
        else:
            ...
            with st.expander("7 Kebiasaan Anak Indonesia Hebat", expanded=True):
                ...
                catatan_kebiasaan = st.text_area("Catatan Kebiasaan Hari Ini:", placeholder="Contoh: Kedisiplinan beribadah dan membawa bekal sehat...")

            with st.expander("8 Dimensi Lulusan", expanded=True):
                st.caption("Keimanan, Kewargaan, Penalaran Kritis, Kreativitas, Mandiri, Gotong Royong, Kebinekaan, Kesehatan.")
                catatan_dimensi = st.text_area("Catatan Dimensi Hari Ini:", placeholder="Contoh: Menunjukkan sikap gotong royong saat piket...")
        ...
        if not students:
            ...
            if not logs:
                ...
                if st.button("⚡ Buat Rangkuman Evaluasi Otomatis", type="primary"):
                    with st.spinner("Sedang menyusun narasi evaluasi perkembangan siswa..."):
                ...
                if summary_text.strip():
                    ...
                    st.info("💡 Klik tombol **'Buat Rangkuman Evaluasi Otomatis'** di atas terlebih dahulu untuk mengunduh berkas Word.")
